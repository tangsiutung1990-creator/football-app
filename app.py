...
st.set_page_config(page_title="足球AI Pro (V22.0)", page_icon="⚽", layout="wide")
...
<style>
    ...
    .team-name { font-weight: bold; font-size: 1.1rem; color: #fff; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; } 
    .team-meta { font-size: 0.75rem; color: #aaa; }
    .score-box { font-size: 1.6rem; font-weight: bold; color: #00ffea; text-align: center; }
    
    /* 6欄緊湊網格 */
    .grid-matrix { display: grid; grid-template-columns: repeat(6, 1fr); gap: 4px; font-size: 0.75rem; margin-top: 6px; text-align: center; }
    .matrix-col { background: #222; padding: 3px; border-radius: 4px; border: 1px solid #333; display: flex; flex-direction: column; justify-content: flex-start; }
    .matrix-header { color: #ff9800; font-weight: bold; font-size: 0.65rem; margin-bottom: 2px; border-bottom: 1px dashed #444; padding-bottom: 1px; white-space: nowrap; }
    .matrix-cell { display: flex; justify-content: space-between; padding: 1px 2px; align-items: center; }
    .cell-label { color: #888; font-size: 0.7rem; }
...
# ================= 主程式 =================
def main():
    st.title("⚽ 足球AI Pro (V22.0 Compact)")
    ...
    for index, row in df.iterrows():
        ...
        prob_o25 = clean_pct(row.get('大球率2.5', 0)) # V22 修正欄位名
        ...
        card_html += f"<div class='matrix-cell'><span class='cell-label'>+1.0</span><span class='cell-val'>{clean_pct(row.get('主+1',0)):.0f}%</span></div></div>"
        ...
        card_html += f"<div class='matrix-cell'><span class='cell-label'>+1.0</span><span class='cell-val'>{clean_pct(row.get('客+1',0)):.0f}%</span></div></div>"
        ...
        card_html += f"<div class='matrix-cell'><span class='cell-label'>大3.5</span><span class='cell-val'>{clean_pct(row.get('大球率3.5',0)):.0f}%</span></div>"
        ...
        card_html += f"<div class='matrix-cell'><span class='cell-label'>賠</span><span style='color:#00e5ff;'>{row.get('主勝賠率','-')}</span></div></div>"
