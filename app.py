...
try:
    ...
    st.error(f"""
    ❌ 缺少必要函式庫，導致 App 無法啟動。
    請確保你的 `requirements.txt` 檔案包含以下內容：
    ```
    gspread
    oauth2client
    pandas
...
def get_google_sheet_data():
    ...
    if json_text:
        try:
            ...
            pk_preview = creds_dict.get('private_key', '')[:10]
            debug_log.append(f"✅ Env Var Loaded (Key starts: {pk_preview}...)")
