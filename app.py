...
st.set_page_config(page_title="足球AI Pro (V21.0)", page_icon="⚽", layout="wide")
...
def main():
    ...
    for index, row in df.iterrows():
        ...
        # 這裡將 HTML 寫成緊湊的單一字串，防止 Markdown 解析錯誤
        ...
        card_html += f"<div class='team-row'><div style='text-align:right;'><div class='team-name'>{row.get('主隊','')} <span style='font-size:0.8rem; color:#888;'>#{row.get('主排名','-')}</span></div><div class='team-meta'>{get_form_html(row.get('主近況'))}</div></div>"
        card_html += f"<div class='score-box'>{row.get('主分','')} - {row.get('客分','')}</div>"
        card_html += f"<div><div class='team-name'><span style='font-size:0.8rem; color:#888;'>#{row.get('客排名','-')}</span> {row.get('客隊','')}</div><div class='team-meta'>{get_form_html(row.get('客近況'))}</div></div></div>"
        
        # Grid
        card_html += f"<div class='grid-matrix'>"
        card_html += f"<div class='matrix-col'><div class='matrix-header'>勝率模型</div><div class='matrix-cell'><span>主</span><span class='{cls_h}'>{prob_h:.0f}%</span></div><div class='matrix-cell'><span>客</span><span class='{cls_a}'>{prob_a:.0f}%</span></div></div>"
        card_html += f"<div class='matrix-col'><div class='matrix-header'>入球模型</div><div class='matrix-cell'><span>大2.5</span><span class='{cls_o25}'>{prob_o25:.0f}%</span></div><div class='matrix-cell'><span>BTTS</span><span class='cell-val'>{clean_pct(row.get('BTTS率',0)):.0f}%</span></div></div>"
        card_html += f"<div class='matrix-col'><div class='matrix-header'>投資價值 (Kelly)</div><div class='matrix-cell'><span>主</span><span class='cell-val'>{clean_pct(row.get('凱利主',0)):.0f}%</span></div><div class='matrix-cell'><span>客</span><span class='cell-val'>{clean_pct(row.get('凱利客',0)):.0f}%</span></div></div>"
        card_html += f"<div class='matrix-col'><div class='matrix-header'>亞盤建議</div><div style='color:#00e5ff; font-weight:bold; margin-top:4px;'>{row.get('亞盤建議','-')}</div></div>"
        card_html += f"<div class='matrix-col'><div class='matrix-header'>真實賠率</div><div class='matrix-cell'><span>主</span><span>{row.get('主勝賠率', '-')}</span></div><div class='matrix-cell'><span>客</span><span>{row.get('客勝賠率', '-')}</span></div></div>"
        card_html += f"</div>"
