...
def get_google_sheet_data():
    ...
    # 1. 嘗試從環境變量 (優先級最高，與 run_me.py 保持一致)
    ...
    if json_text:
        try:
            ...
            # 記錄 Key 的狀態 (不顯示完整 Key，只顯示頭尾)
            pk = creds_dict.get('private_key', '')
            pk_preview = f"{pk[:10]}...{pk[-10:]}" if pk else "Empty"
            debug_log.append(f"✅ Env Var Loaded (Email: {creds_dict.get('client_email')}, Key: {pk_preview})")
            
        except Exception as e:
            debug_log.append(f"❌ Env Var Error: {e}")

    # 2. 嘗試從 Secrets (Streamlit Cloud 常用)
    if not creds:
        try:
            if hasattr(st, "secrets") and "gcp_service_account" in st.secrets:
                ...
                creds = ServiceAccountCredentials.from_json_keyfile_dict(creds_dict, scope)
                debug_log.append(f"✅ Secrets Loaded (Email: {creds_dict.get('client_email')})")
        except Exception as e:
    ...
    if creds:
        try:
            ...
            debug_log.append(f"🔥 Auth Success but Connect Fail: {e}")
