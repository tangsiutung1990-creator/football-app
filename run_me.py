...
def get_google_spreadsheet():
    ...
    if json_text:
        try:
            ...
            if json_text.startswith("-----BEGIN"):
                print("❌ [嚴重錯誤] 你貼的是 Private Key，不是 JSON 格式！")
                return None
    ...
    if creds:
        try:
            ...
            print(f"❌ Google Sheet 連接最終失敗 (Auth Error): {e}")
            return None
...
# ================= 主流程 =================
def main():
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 🚀 V40.6 (Fix Auth) (Connected)")
